...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        if r_squared(y, fit) > rSquared:
            degree = len(model) + 1
            bestfit = fit
            rSquared = r_squared(y,fit)
    pylab.figure()
    
    pylab.scatter(x,y)
    pylab.plot(x, bestfit, 'r')
    pylab.title(str(degree)+' degree regression model with R Squared of '+\
    str(round(rSquared,3)))
